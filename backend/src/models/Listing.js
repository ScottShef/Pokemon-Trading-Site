...
// --- Grade Definitions ---
// (All this code remains exactly the same)
...
const CGC_GRADES = ['Pristine 10', 'Gem Mint 10', 'Mint+ 9.5', 'Mint 9', 'Near Mint/Mint+ 8.5', 'Near Mint/Mint 8', 'Near Mint+ 7.5', 'Near Mint 7', 'Excellent/Near Mint+ 6.5', 'Excellent/Near Mint 6', 'Very Good/Excellent+ 5.5', 'Very Good/Excellent 5', 'Very Good+ 4.5', 'Very Good 4', 'Good/Very Good+ 3.5', 'Good/Very Good 3', 'Good+ 2.5', 'Good 2', 'Fair 1.5', 'Poor 1'];
const BECKETT_GRADES = ['Black Label 10', 'Pristine 10', 'Gem Mint 9.5', '9', '8.5', '8', '7.5', '7', '6.5', '6', '5.5', '5', '4.5', '4', '3.5', '3', '2.5', '2', '1.5', '1'];

// --- Main Listing Schema ---
// (Schema definition remains exactly the same)
...
// (This hook remains exactly the same)
ListingSchema.pre('validate', function(next) {
  if (this.listingType === 'raw') {
    ...
    const { company, grade } = this.gradedData;
    let validGrades = [];
    if (!company || !grade) {
        return next(new Error('Graded listings must include both a company and a grade.'));
    }
    switch (company) {
    ...
    if (!validGrades.includes(grade)) {
      return next(new Error(`'${grade}' is not a valid grade for ${company}.`));
    }
  } else {
    this.rawCondition = undefined;
    this.gradedData = undefined;
...
export default Listing; // Changed from module.exports to export default
